...
declare type ExplicitlyOptionalProps2<T> = ExplicitlyOptionalProps<T> | 'id' | '_id' | 'uuid';
declare type IsOptional<T, K extends keyof T> = T[K] extends Collection<any> ? true : T[K] extends Function ? true : K extends symbol ? true : K extends ExplicitlyOptionalProps2<T> ? true : false;
...
export declare type Loaded<T, L extends string = never> = T & {
    [K in keyof T]: K extends Prefix<L> ? LoadedLoadable<Defined<T[K]>, Loaded<ExtractType<Defined<T[K]>>, Suffix<L>>> : T[K];
};
